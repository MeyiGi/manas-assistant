"""
src/infrastructure/wiring/_grades.py
=====================================
Composition root for the Grades bounded context.

This file is OWNED by the team working on the grades context.
...
Implementation checklist:
  ...
  [ ] Return populated GradesContainer
"""
...
class GradesContainer:
    """
    Holds wired use-case instances for the Grades context.
...
def build_grades(settings: Settings, shared: SharedInfrastructure) -> GradesContainer:
    """
    Wire all adapters and use cases for the Grades bounded context.

    Example implementation (uncomment as you build):
        repo = SqlGradesRepository(shared.db_session_factory)
        notifier = TelegramNotificationAdapter(settings.notifications)
        use_case = CreateGradesUseCase(repo=repo, notifier=notifier, clock=shared.clock)
        return GradesContainer(create_grades=use_case)
    """
